...
function CountryListComponent(props) {
    ...
    return (
        <InputGroup className="mb-3 d-flex">
            ...
            <Form.Select className="selected_country" onChange={(e) => props.handleCountrySelect(e.target.value)}
            aria-label="Default select example"
            defaultValue={props.country}>
                <option className="" value="">Open this select menu</option>
                {props.countryList
                    .filter(country => country.name.includes(inputValue))
                    .map(country => (<option key={country.key} id={country.key} value={country.key}>{country.name}</option>))}
            </Form.Select>
        </InputGroup>
    );
}
